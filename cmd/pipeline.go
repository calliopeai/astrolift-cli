...
var pipelineRunCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// First look up the pipeline ID by name
		listQuery := `query FindPipeline($limit: Int) { astroliftPipelines(limit: $limit) { id name } }`
		var listResp struct {
			Pipelines []struct {
				ID   string `json:"id"`
				Name string `json:"name"`
		...
		if err := client.GraphQL(ctx, listQuery, map[string]interface{}{"limit": 200}, &listResp); err != nil {
...
mutation TriggerPipelineRun($input: TriggerPipelineRunInput!) {
  triggerPipelineRun(input: $input) {
    ...
    data { id runNumber status }
		...
		input := map[string]interface{}{"pipelineId": pipelineID}
		if pipelineRunBranch != "" {
			input["ref"] = pipelineRunBranch
		}
		...
		if err := client.GraphQL(ctx, mutation, map[string]interface{}{"input": input}, &mutResp); err != nil {
...
query ListPipelineRuns($pipelineId: ID, $limit: Int) {
  astroliftPipelineRuns(pipelineId: $pipelineId, limit: $limit) {
    id runNumber status triggerKind triggerRef startedAt finishedAt
		...
		vars := map[string]interface{}{"limit": pipelineRunsLimit}
		if pipelineRunsPipeline != "" {
			vars["pipelineId"] = pipelineRunsPipeline
		}
...
var pipelineCancelCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		mutation := `
mutation CancelPipelineRun($id: ID!) {
  cancelPipelineRun(id: $id) {
    ok
    errors { code message }
  }
}`
		...
		if err := client.GraphQL(ctx, mutation, map[string]interface{}{"id": runID}, &resp); err != nil {
...
var pipelineLogsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return notImplemented(cmd,
			"pipeline logs — log streaming requires StepRun.log_excerpt to be wired; "+
				"use the web UI Logs tab to view logs until this is implemented")
	},
}
...
func init() {
	...
	pipelineRunsCmd.Flags().StringVar(&pipelineRunsPipeline, "pipeline", "", "Filter by pipeline ID or name")
